...
export default function CsvBatchImport() {
  ...
  const processRow = async (row: CsvRow): Promise<CsvRow> => {
    ...
    try {
      let estimate = await requestEstimate(measurementAddress);
      if (!estimate.solarFound && row.mailing_address?.trim()) {
        estimate = await requestEstimate(row.mailing_address);
      }
      if (!estimate.solarFound && isCollegeSta(row.city)) {
        estimate = await requestEstimate(buildPropertyAddress(row, "College Station"));
      }
